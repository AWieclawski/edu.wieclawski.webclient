...
public class NbpIntegrationServiceImpl implements NbpARateService {
	...
	private final String NBP_API_URL = "http://api.nbp.pl/api";
	...
	public List<NbpARateDto> getATypeRateByDateAndSymbol(LocalDate publicationDate, String currencySymbol) {
		String path = getARatePathWithDateAndSymbol(publicationDate, currencySymbol);
		var response = this.webclient.get()
				.uri(uriBuilder -> uriBuilder.path(path).queryParam("format", RATES_DATA_FORMAT).build())
				.accept(MediaType.APPLICATION_JSON).retrieve()
				.onStatus(httpStatus -> !httpStatus.is2xxSuccessful(), handleErrorResponse())
				.bodyToMono(new ParameterizedTypeReference<NbpResponseDto<List<NbpARateDto>>>() {
				}).block();

		return response != null ? response.getRates() : null;
	}

	private String getARatePathWithDateAndSymbol(LocalDate publicationDate, String currencySymbol) {

		return buildPathFromVariablesList(List.of(CONVERSION_RATES_PATH.toLowerCase(), RATES_TABLE_TYPE.toLowerCase(),
				currencySymbol.toLowerCase(), publicationDate.format(DATE_TIME_FORMATTER)));
	}

	private String buildPathFromVariablesList(List<String> pathVariables) {
		URIBuilder builder = new URIBuilder();
		String path = "";

		for (String tmp : pathVariables) {
		...
		throw new RuntimeException("NBP url buildeing problem");
	}
	...
	private ExchangeFilterFunction loggRequest() {
		...
		return ExchangeFilterFunction.ofRequestProcessor(clientRequest -> {
			...
			clientRequest.headers().forEach(
					(name, values) -> values.forEach(value -> LOG.info("Header name={}, value={}", name, value)));
